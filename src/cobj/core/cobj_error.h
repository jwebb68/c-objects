...
typedef enum cobj_ErrCode_e {
    ...
    cobj_ErrorCode_EINVAL,
    cobj_ErrorCode_EINVALID,
} cobj_ErrorCode;
...
#    define COBJ_ERROR_RAISE(self, code) cobj_Error_raise_(self, code, __LINE__, __FILE__)

#    define COBJ_ERROR_RAISE_IF(cond, self, code) \
        do { \
            if (!(cond)) { COBJ_ERROR_RAISE((self), (code)) } \
        } while (0)
